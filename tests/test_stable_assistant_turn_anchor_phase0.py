...
def _anchor_source() -> str:
    return "\n".join(
        ...
        for path in (ANCHORS_JS.parent / "model.js", ANCHORS_JS.parent / "activity-scene.js", ANCHORS_JS)
    )
...
def test_phase0_scaffold_is_loaded_before_current_rendering_modules():
    ...
    ui_pos = html.index('static/modules/ui/index.js?v=__WEBUI_VERSION__')
    ...
    ui_src = _read(UI_JS)
    sw_src = _read(SW_JS)
    assert "'./static/modules/assistant-turn-anchors/index.js' + VQ" in sw_src
    assert "'./static/modules/assistant-turn-anchors/model.js'" in sw_src
    assert "'./static/modules/assistant-turn-anchors/activity-scene.js'" in sw_src
...
def test_anchor_entrypoint_exports_a_frozen_api_without_assembly_state():
    assert NODE, "node is required for assistant turn anchor module tests"
    script = f"""
const {{HermesAssistantTurnAnchors:api}} = await import({json.dumps(ANCHORS_JS.as_uri())});
console.log(JSON.stringify({{
  apiFrozen:Object.isFrozen(api),
  globalMatches:globalThis.HermesAssistantTurnAnchors === api,
  partsPresent:Object.prototype.hasOwnProperty.call(globalThis, 'HermesAssistantTurnAnchorParts'),
}}));
"""
    result = subprocess.run([NODE, "--input-type=module", "-e", script], text=True, capture_output=True, check=False)
    assert result.returncode == 0, result.stderr
