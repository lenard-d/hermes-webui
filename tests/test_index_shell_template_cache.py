"""Regression tests for the app-shell template cache (render_index_shell_base).
...
__MAX_UPLOAD_BYTES__) on every request. render_index_shell_base() caches the
...
def test_cached_render_is_byte_identical_to_old_inline():
    ...
    new = shell.render_index_shell_base().replace(
...
def test_csrf_token_varies_per_request():
    a = shell.render_index_shell_base().replace(
        "__CSRF_TOKEN_JSON__", json.dumps("AAA")
    )
    b = shell.render_index_shell_base().replace(
...
def test_second_call_returns_cached_object():
    ...
    first = shell.render_index_shell_base()
    second = shell.render_index_shell_base()
...
def test_cache_invalidates_on_mtime_change(tmp_path, monkeypatch):
    ...
    shell.render_index_shell_base()
    sig_before = shell._INDEX_SHELL_CACHE["base"][0]

    future = time.time() + 5
    os.utime(fake, (future, future))
    shell.render_index_shell_base()
    sig_after = shell._INDEX_SHELL_CACHE["base"][0]
...
def test_cache_invalidates_on_index_path_change(tmp_path, monkeypatch):
    ...
    assert "alpha" in shell.render_index_shell_base()
    selected["path"] = second
    rendered = shell.render_index_shell_base()
