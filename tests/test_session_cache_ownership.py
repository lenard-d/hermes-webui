...
def test_get_session_evicts_cached_object_with_wrong_session_id(tmp_path, monkeypatch):
    ...
    session_records.SESSIONS["requested-tip"] = wrong
    ...
    assert session_records.SESSIONS["requested-tip"] is loaded
...
def test_get_session_metadata_only_evicts_cached_object_with_wrong_session_id(tmp_path, monkeypatch):
    ...
    requested.save()
    session_records.SESSIONS["requested-tip"] = Session(
        session_id="older-segment", title="Older Segment"
    )
    ...
    assert loaded.session_id == "requested-tip"
    assert loaded.title == "Requested Tip"
    assert session_records.SESSIONS.get("requested-tip") is None
...
def test_handle_chat_steer_evicts_mismatched_cached_agent(monkeypatch):
    ...
    live_controls._handle_chat_steer(
        handler, {"session_id": "requested", "text": "please steer"}
    )
