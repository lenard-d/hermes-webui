...
from api.onboarding.catalog import (
    PROVIDER_CATEGORIES,
    SUPPORTED_PROVIDER_SETUPS,
    build_setup_catalog,
)
from api.onboarding.setup import apply_onboarding_setup
...
class TestProviderCatalog:
    ...
    def test_all_providers_have_category(self):
        for pid, meta in SUPPORTED_PROVIDER_SETUPS.items():
            assert "category" in meta, f"Provider {pid} missing 'category'"

    def test_categories_are_valid(self):
        valid_ids = {c["id"] for c in PROVIDER_CATEGORIES}
        for pid, meta in SUPPORTED_PROVIDER_SETUPS.items():
            cat = meta["category"]
    ...
    def test_local_providers_require_base_url(self):
        for pid in ["ollama", "lmstudio", "custom"]:
            assert SUPPORTED_PROVIDER_SETUPS[pid]["requires_base_url"]
    ...
    def test_google_uses_gemini_key(self):
        ...
        assert "gemini" in SUPPORTED_PROVIDER_SETUPS
        assert "google" not in SUPPORTED_PROVIDER_SETUPS
    ...
    def test_gemini_model_list_is_populated(self):
        ...
        """
        gemini = SUPPORTED_PROVIDER_SETUPS["gemini"]
...
class TestProviderCategoryOrder:
    ...
    def test_categories_sorted_by_order(self):
        orders = [c["order"] for c in PROVIDER_CATEGORIES]
        assert orders == sorted(orders)

    def test_category_ids(self):
        ids = {c["id"] for c in PROVIDER_CATEGORIES}
        assert ids == {"easy_start", "self_hosted", "specialized"}

    def test_three_categories(self):
        assert len(PROVIDER_CATEGORIES) == 3


# ── Backend: _build_setup_catalog ────────────────────────────────────────

class TestBuildSetupCatalog:
    def test_catalog_has_categories_key(self, tmp_path):
        cfg = {"model": {"provider": "openrouter", "default": "anthropic/claude-sonnet-4.6"}}
        catalog = build_setup_catalog(cfg, hermes_home=tmp_path, oauth_authenticated=lambda *_: False)
        assert "categories" in catalog
        assert isinstance(catalog["categories"], list)

    def test_catalog_categories_have_providers_list(self, tmp_path):
        cfg = {"model": {"provider": "openrouter", "default": "anthropic/claude-sonnet-4.6"}}
        catalog = build_setup_catalog(cfg, hermes_home=tmp_path, oauth_authenticated=lambda *_: False)
    ...
    def test_catalog_providers_have_category_field(self, tmp_path):
        cfg = {"model": {"provider": "openrouter", "default": "anthropic/claude-sonnet-4.6"}}
        catalog = build_setup_catalog(cfg, hermes_home=tmp_path, oauth_authenticated=lambda *_: False)
        for p in catalog["providers"]:
            assert "category" in p

    def test_catalog_providers_sorted_by_category(self, tmp_path):
        cfg = {"model": {"provider": "openrouter", "default": "anthropic/claude-sonnet-4.6"}}
        catalog = build_setup_catalog(cfg, hermes_home=tmp_path, oauth_authenticated=lambda *_: False)
        cat_order = {c["id"]: c["order"] for c in PROVIDER_CATEGORIES}
        prev_order = -1
        for p in catalog["providers"]:
    ...
    def test_catalog_quick_flag_on_openrouter(self, tmp_path):
        cfg = {"model": {"provider": "openrouter", "default": "anthropic/claude-sonnet-4.6"}}
        catalog = build_setup_catalog(cfg, hermes_home=tmp_path, oauth_authenticated=lambda *_: False)
        orow = next(p for p in catalog["providers"] if p["id"] == "openrouter")
        assert orow["quick"] is True

    def test_catalog_no_quick_flag_on_others(self, tmp_path):
        cfg = {"model": {"provider": "openrouter", "default": "anthropic/claude-sonnet-4.6"}}
        catalog = build_setup_catalog(cfg, hermes_home=tmp_path, oauth_authenticated=lambda *_: False)
        for p in catalog["providers"]:
            if p["id"] != "openrouter":
...
class TestApplyBaseURL:
    ...
    def test_requires_base_url_writes_user_url(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("api.onboarding.setup.get_config_path", lambda: config_path)
