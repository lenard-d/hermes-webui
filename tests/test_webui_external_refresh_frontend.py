...
def _session_owner_source(name: str) -> str:
    return (SESSIONS_MODULES / name).read_text(encoding="utf-8")


SESSION_LIST_REFRESH_JS = _session_owner_source("session-list-refresh.js")
SESSION_SOURCE_JS = _session_owner_source("session-source.js")
SIDEBAR_SESSION_EVENTS_JS = _session_owner_source("sidebar-session-events.js")
SESSION_PROFILE_SCOPE_JS = _session_owner_source("session-profile-scope.js")
SESSION_LIST_RECONCILIATION_JS = _session_owner_source("session-list-reconciliation.js")
EXISTING_SESSION_LOAD_JS = _session_owner_source("existing-session-load.js")
COMPOSER_DRAFTS_JS = _session_owner_source("composer-drafts.js")
TRANSCRIPT_LOADING_JS = _session_owner_source("transcript-loading.js")
SESSION_LOAD_RECOVERY_JS = _session_owner_source("session-load-recovery.js")
...
def test_load_session_supports_force_reload_for_external_refresh():
    assert "async function loadSession(sid)" in EXISTING_SESSION_LOAD_JS
    assert "const opts = arguments[1] || {};" in EXISTING_SESSION_LOAD_JS
    assert "const forceReload = !!opts.force" in EXISTING_SESSION_LOAD_JS
    assert "if(currentSid===sid && !forceReload" in EXISTING_SESSION_LOAD_JS
    assert "loadSession(sid, {force:true" in SESSION_LIST_REFRESH_JS


def test_active_session_external_refresh_uses_metadata_then_force_reload():
    assert "function ensureActiveSessionExternalRefreshPoll()" in SESSION_LIST_REFRESH_JS
    assert "async function refreshActiveSessionIfExternallyUpdated(reason)" in SESSION_LIST_REFRESH_JS
    assert "messages=0&resolve_model=0" in SESSION_LIST_REFRESH_JS
    assert "if(remoteCount !== localCount)" in SESSION_LIST_REFRESH_JS
    assert "else if(remoteLast > localLast)" in SESSION_LIST_REFRESH_JS
    assert "if(S.busy || S.activeStreamId) return;" in SESSION_LIST_REFRESH_JS
    assert "document.hidden" in SESSION_LIST_REFRESH_JS
    assert "externalRefreshReason:reason||'poll'" in SESSION_LIST_REFRESH_JS
...
def test_active_session_external_refresh_skips_destructive_reload_on_metadata_only_bump():
    ...
    sidebar list.
    """
    assert "remoteCount > localCount || remoteLast > localLast" not in SESSION_LIST_REFRESH_JS
    assert "if(remoteCount !== localCount){" in SESSION_LIST_REFRESH_JS
    assert "await loadSession(sid, {force:true, externalRefreshReason:reason||'poll', keepStaleUntilLoaded:_keepStaleUntilLoaded});" in SESSION_LIST_REFRESH_JS
    assert "}else if(remoteLast > localLast){" in SESSION_LIST_REFRESH_JS
    assert "S.session.last_message_at = remoteLast" in SESSION_LIST_REFRESH_JS
    assert "if(data.session.updated_at) S.session.updated_at = data.session.updated_at;" in SESSION_LIST_REFRESH_JS
...
def test_active_session_external_refresh_force_reloads_on_count_decrease():
    ...
    assert "if(remoteCount !== localCount){" in SESSION_LIST_REFRESH_JS
    assert "if(remoteCount > localCount){" not in SESSION_LIST_REFRESH_JS
...
def test_webui_source_never_counts_as_external_session():
    assert "function _isWebUiSourceSession(session)" in SESSION_SOURCE_JS
    assert "if (!session || _isWebUiSourceSession(session)) return false;" in SESSION_SOURCE_JS
    external_start = SESSION_SOURCE_JS.index("function _isExternalSession(session)")
    external_body = SESSION_SOURCE_JS[external_start : external_start + 300]
...
def test_active_session_external_refresh_has_focus_and_visibility_hooks():
    assert "visibilitychange" in SESSION_LIST_REFRESH_JS
    assert "window.addEventListener('focus'" in SESSION_LIST_REFRESH_JS
    assert "ensureActiveSessionExternalRefreshPoll" in SESSION_LIST_REFRESH_JS
...
def test_session_time_refresh_has_own_visibility_hook():
    ...
    start = SESSION_LIST_REFRESH_JS.find("function ensureSessionTimeRefreshPoll()")
    assert start != -1
    block = SESSION_LIST_REFRESH_JS[start:start + 900]
    assert "_sessionTimeRefreshVisibilityHandler" in SESSION_LIST_REFRESH_JS
...
def test_session_list_external_refresh_uses_sse_invalidation_not_polling():
    ...
    assert "async function refreshSessionList(reason='manual', opts={})" in SESSION_LIST_REFRESH_JS
    assert "let _sessionListRefreshPendingRequest = null;" in SESSION_LIST_REFRESH_JS
    assert "function _mergeSessionListRefreshOptions(prev, next)" in SESSION_LIST_REFRESH_JS
    assert "renderSessionList({deferWhileInteracting:!force})" in SESSION_LIST_REFRESH_JS
    assert "const refreshActive = !!(opts && opts.refreshActive)" in SESSION_LIST_REFRESH_JS
    assert "if(refreshActive) await refreshActiveSessionIfExternallyUpdated(reason||'session-list')" in SESSION_LIST_REFRESH_JS
    assert "_sessionListRefreshPendingRequest = {" in SESSION_LIST_REFRESH_JS
    assert "setTimeout(()=>{void refreshSessionList(pendingRequest.reason, pendingRequest.opts);},300)" in SESSION_LIST_REFRESH_JS

    assert "function _refreshSessionListAfterSidebarResume(reason)" in SIDEBAR_SESSION_EVENTS_JS
    assert "_sessionEventsNeedsRefreshOnOpen = false" in SIDEBAR_SESSION_EVENTS_JS
    assert "void refreshSessionList(reason, {force:true})" in SIDEBAR_SESSION_EVENTS_JS
    assert "function ensureSessionEventsSSE()" in SIDEBAR_SESSION_EVENTS_JS
    assert "new EventSource('api/sessions/events')" in SIDEBAR_SESSION_EVENTS_JS
    assert "addEventListener('sessions_changed'" in SIDEBAR_SESSION_EVENTS_JS
    assert "function _scheduleSessionEventsRefresh(reason, opts={})" in SIDEBAR_SESSION_EVENTS_JS
    assert "let _sessionEventsRefreshPendingRequest = null;" in SIDEBAR_SESSION_EVENTS_JS
    assert "_sessionEventsNeedsRefreshOnOpen = true" in SIDEBAR_SESSION_EVENTS_JS
    assert "void _refreshSessionListAfterSidebarResume('focus')" in SIDEBAR_SESSION_EVENTS_JS
    assert "void _refreshSessionListAfterSidebarResume('visible')" in SIDEBAR_SESSION_EVENTS_JS
    assert "void _refreshSessionListAfterSidebarResume('reconnect')" in SIDEBAR_SESSION_EVENTS_JS
    assert "ensureSessionEventsSSE();" in SIDEBAR_SESSION_EVENTS_JS
    assert "document._hermesSessionEventsVisibilityHook" in SIDEBAR_SESSION_EVENTS_JS
    ensure_fn = SIDEBAR_SESSION_EVENTS_JS[SIDEBAR_SESSION_EVENTS_JS.find("function ensureSessionEventsSSE()") :]
    ...
    assert "_sessionListExternalRefreshMs" not in SESSION_LIST_REFRESH_JS
    ...
    assert "function _sessionEventTargetsActiveSession(payload)" in SIDEBAR_SESSION_EVENTS_JS
    assert "typeof payload.session_id === 'string'" in SIDEBAR_SESSION_EVENTS_JS
...
def test_session_events_refresh_forces_hidden_sidebar_render_from_event_path():
    ...
    merge_body = (
        _function_body(SESSION_LIST_REFRESH_JS, "_mergeSessionListRefreshOptions")
        if "function _mergeSessionListRefreshOptions" in SESSION_LIST_REFRESH_JS
        else "function _mergeSessionListRefreshOptions(prev, next){ return {...(prev||{}), ...(next||{})}; }"
    )
    functions = "\n\n".join(
        [
            merge_body,
            _function_body(SESSION_LIST_REFRESH_JS, "refreshSessionList"),
            _function_body(SIDEBAR_SESSION_EVENTS_JS, "_scheduleSessionEventsRefresh"),
        ]
    )
    script = textwrap.dedent(
        ...
        {functions}
        const scheduleSessionEventsRefresh = _scheduleSessionEventsRefresh;
        (async() => {{
...
def test_session_list_external_refresh_forced_resume_survives_hidden_inflight_refresh():
    ...
    functions = "\n\n".join(
        (
            ...
            _function_body(SESSION_LIST_REFRESH_JS, "refreshSessionList"),
            _function_body(SIDEBAR_SESSION_EVENTS_JS, "_scheduleSessionEventsRefresh"),
            _function_body(SIDEBAR_SESSION_EVENTS_JS, "_refreshSessionListAfterSidebarResume"),
        )
    )
    script = textwrap.dedent(
        f"""
...
def test_session_event_profile_filter_tolerates_default_root_aliases():
    assert "function _profileMatchesActiveProfile(profile, activeProfile)" in SESSION_PROFILE_SCOPE_JS
    assert "return eventName === 'default' && !!S.activeProfileIsDefault;" in SESSION_PROFILE_SCOPE_JS
    assert "function _sessionEventProfilesMatch(eventProfile, activeProfile)" in SESSION_PROFILE_SCOPE_JS
    assert "return _profileMatchesActiveProfile(eventProfile, activeProfile);" in SESSION_PROFILE_SCOPE_JS
...
def test_session_list_render_signature_serializes_full_rows_not_a_narrow_allowlist():
    ...
    """
    start = SESSION_LIST_RECONCILIATION_JS.find("function _sessionListRenderSignature()")
    assert start != -1
    block = SESSION_LIST_RECONCILIATION_JS[start:start + 900]
...
def test_session_list_render_signature_does_not_skip_recovering_from_skeleton_or_error():
    ...
    """
    start = SESSION_LIST_RECONCILIATION_JS.find("function _applySessionListPayload(")
    assert start != -1
    body = SESSION_LIST_RECONCILIATION_JS[start:start + 12000]
    assert "const _hadSessionListSkeleton = sessionListViewBindings._sessionListSkeletonActive;" in body
    assert "const _hadSessionListLoadError = !!sessionListCoordination.loadError;" in body
    ...
    assert body.index("const _hadSessionListSkeleton") < body.index("sessionListViewBindings._sessionListSkeletonActive = false;")
    assert body.index("const _hadSessionListLoadError") < body.index("sessionListCoordination.loadError = null;")
...
def test_force_reload_clears_stale_blocking_prompts_immediately():
    ...
    """
    assert "hideApprovalCard(forceReload)" in EXISTING_SESSION_LOAD_JS
    assert "hideClarifyCard(forceReload, forceReload?'external-refresh':'dismissed')" in EXISTING_SESSION_LOAD_JS
...
def test_same_session_force_reload_preserves_non_empty_composer_input():
    ...
    """
    assert "function _restoreComposerDraft(draft, targetSid, opts={})" in COMPOSER_DRAFTS_JS
    assert "const preserveActiveInput = !!(opts && opts.preserveActiveInput);" in COMPOSER_DRAFTS_JS
    assert "if (preserveActiveInput && current && current !== text) return;" in COMPOSER_DRAFTS_JS
    assert "_restoreComposerDraft(_draft, sid, {preserveActiveInput:!!opts.preserveActiveInput || (currentSid===sid&&forceReload)});" in EXISTING_SESSION_LOAD_JS
...
def test_same_session_force_reload_keeps_loaded_transcript_width_hint():
    ...
    assert "let _sameSessionForceReloadHint = null;" in TRANSCRIPT_LOADING_JS
    assert "function _captureSameSessionForceReloadHint(sid)" in TRANSCRIPT_LOADING_JS
    assert "if(!sid || _sameSessionForceReloadHint.session_id===sid) _sameSessionForceReloadHint=null;" in TRANSCRIPT_LOADING_JS
    assert "loaded_renderable_count:loadedRenderableCount" in TRANSCRIPT_LOADING_JS
    assert "message_count:knownMessageCount" in TRANSCRIPT_LOADING_JS
    assert "truncated:!!transcriptWindowState.messagesTruncated" in TRANSCRIPT_LOADING_JS
    assert "function _messageReloadLimitForSession(sid)" in TRANSCRIPT_LOADING_JS
    assert "if(!hint.truncated) return null;" in TRANSCRIPT_LOADING_JS
    assert "const appendedMessageCount=Math.max(0,currentMessageCount-previousMessageCount);" in TRANSCRIPT_LOADING_JS
    assert "return Math.max(INITIAL_MESSAGE_LIMIT,loadedRenderableCount,loadedMessageCount+appendedMessageCount);" in TRANSCRIPT_LOADING_JS
    assert "const reloadLimit = _messageReloadLimitForSession(sid);" in TRANSCRIPT_LOADING_JS
    ...
    assert "const boundedReloadLimit = (reloadLimit && reloadLimit <= transcriptWindowState.msgLimitMax) ? reloadLimit : null;" in TRANSCRIPT_LOADING_JS
    assert "const reloadLimitParam = boundedReloadLimit ? `&msg_limit=${boundedReloadLimit}` : '';" in TRANSCRIPT_LOADING_JS
    assert "if (_ownsLoad()) _clearSameSessionForceReloadHint(sid);" in TRANSCRIPT_LOADING_JS

    load_body = _function_body(EXISTING_SESSION_LOAD_JS, "loadSession")
    ...
    restore_body = _function_body(SESSION_LOAD_RECOVERY_JS, "_restoreLoadedSession")
...
def test_same_width_force_reload_invalidates_visible_message_cache():
    ...
    ensure_body = _function_body(TRANSCRIPT_LOADING_JS, "_ensureMessagesLoaded")
