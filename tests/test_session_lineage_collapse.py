"""Regression tests for the focused sidebar lineage owners."""
import json
...
SESSION_OWNER_IMPORTS = """
import {
  _activeSessionIdForSidebar,
  _sessionIdFromLocation,
} from './static/modules/sessions/session-navigation.js';
import {
...
} from './static/modules/sessions/session-lineage.js';
import {
  _attachChildSessionsToSidebarRows,
  _syncSidebarExpansionForActiveSession,
} from './static/modules/sessions/session-child-attachment.js';
import {
...
} from './static/modules/sessions/session-lineage-report.js';
import {
  _renderSidebarRowsFromRawSessions,
  _scopedSidebarReferenceRows,
} from './static/modules/sessions/sidebar-row-state.js';
import {
  _sessionDisplayTitle,
  _sessionTitleIsDefaultWebUI,
  _sessionTitleTags,
} from './static/modules/sessions/session-display.js';
import { _sessionTimestampMs } from './static/modules/sessions/session-time.js';
import { _markSessionCompletionUnread } from './static/modules/sessions/session-unread.js';
import {
...
} from './static/modules/sessions/sidebar-store.js';
globalThis.S = { session: null, messages: [] };
"""


def _run_node(source: str) -> str:
    # Run the actual native modules rather than extracting function text from a
    # synthetic concatenation of the complete sessions package.
...
def _session_owner_source(name: str) -> str:
    return (REPO_ROOT / "static" / "modules" / "sessions" / name).read_text(
        encoding="utf-8"
    )


def test_sidebar_lineage_collapse_keeps_latest_tip_and_counts_segments():
    source = SESSION_OWNER_IMPORTS + """
const sessions = [
...
def test_sidebar_reference_rows_suppress_source_filtered_archived_parent_child():
    ...
    """
    source = SESSION_OWNER_IMPORTS + """
sidebarStateBindings._showArchived = false;
sidebarStateBindings._activeProject = null;
sidebarStateBindings._sidebarReferenceSessions = [{session_id:'parent', title:'Archived parent', archived:true, updated_at:10, last_message_at:10, source:'webui'}];
...
def test_sidebar_reference_rows_do_not_suppress_cross_project_child():
    ...
    source = SESSION_OWNER_IMPORTS + """
sidebarStateBindings._showArchived = false;
sidebarStateBindings._activeProject = 'projA';
sidebarStateBindings._sidebarReferenceSessions = [{session_id:'parent', title:'Archived parent (projB)', archived:true, project_id:'projB', updated_at:10, last_message_at:10, source:'webui'}];
...
def test_nested_fork_bubbles_parent_attention_state():
    source = SESSION_OWNER_IMPORTS + """
_markSessionCompletionUnread('fork1', 0);
const parent = {session_id:'parent', title:'Parent', updated_at:10, last_message_at:10};
const fork = {
  session_id:'fork1',
  title:'Fork',
...
def test_sidebar_lineage_segment_badge_is_detailed_density_only_and_localized():
    presentation = _session_owner_source("sidebar-row-presentation.js")
    css = family_source("style")
    assert "session-lineage-count" in presentation
    assert "const density=(window._sidebarDensity==='detailed'?'detailed':'compact');" in presentation
    assert "const showLineageMetadata=density==='detailed';" in presentation
    assert "const segmentCount=showLineageMetadata?_sessionSegmentCount(s):0;" in presentation
    assert "const needsLineageReport=showLineageMetadata?_lineageReportNeedsFetch(s,lineageKey,segmentCount):false;" in presentation
    assert "const lineageSegments=showLineageMetadata?_lineageSegmentsForRender(s,lineageKey,needsLineageReport):[];" in presentation
    assert "const canExpandLineageSegments=showLineageMetadata&&Boolean(" in presentation
    assert "t('session_meta_segments', segmentCount)" in presentation
    assert "titleRow.appendChild(segmentCountEl);" in presentation
    assert ".session-lineage-count{" in css


def test_lineage_segment_expansion_static_contract():
    store = _session_owner_source("sidebar-store.js")
    report = _session_owner_source("session-lineage-report.js")
    reconciliation = _session_owner_source("session-list-reconciliation.js")
    presentation = _session_owner_source("sidebar-row-presentation.js")
    gestures = _session_owner_source("sidebar-row-gestures.js")
    session_load = _session_owner_source("existing-session-load.js")
    css = family_source("style")
    assert "const _expandedLineageKeys = new Set();" in store
    assert "const _lineageReportCache = new Map();" in store
    assert "const _lineageReportInflight = new Map();" in store
    assert "_pruneLineageReportCacheToVisibleSessions(sidebarStateBindings._allSessions);" in reconciliation
    assert "session-lineage-count,.session-lineage-segments,.session-lineage-segment" in gestures
    assert "segmentCountEl.setAttribute('aria-expanded'" in presentation
    assert "_expandedLineageKeys.has(lineageKey)" in presentation
    assert "_expandedLineageKeys.add(lineageKey)" in presentation
    assert "_expandedLineageKeys.delete(lineageKey)" in presentation
    assert "_fetchLineageReportForRow(s,lineageKey).then" in presentation
    assert presentation.count("_fetchLineageReportForRow(s,lineageKey).then(()=>renderSessionListFromCache());") == 2
    assert "'/api/session/lineage/report?session_id='" in report
    assert "encodeURIComponent(s.session_id)" in report
    assert "className='session-lineage-segments'" in presentation
    assert "className='session-lineage-segment'" in presentation
    assert "const segTitle=_sessionDisplayTitle(seg)||t('session_lineage_segment_untitled');" in presentation
    assert "row.title=t('session_lineage_segment_open');" in presentation
    assert "await _openSidebarSession(seg, {skipLineageResolve:true});" in presentation
    assert "const openChildSession=async(childSession)=>{" in presentation
    assert "await _openSidebarSession(childSession, {skipLineageResolve:true});" in presentation
    assert "if(!opts.skipLineageResolve && typeof _resolveSessionIdFromSidebarLineage==='function'){" in session_load
...
def test_lineage_report_fetch_uses_endpoint_once_and_caches_result():
    source = SESSION_OWNER_IMPORTS + """
sidebarStateBindings._lineageReportCacheGeneration = 0;
const calls = [];
globalThis.api = function api(path) {
  calls.push(path);
...
def test_sidebar_search_and_rows_use_read_only_display_title():
    ...
    display = _session_owner_source("session-display.js")
    search = _session_owner_source("session-search.js")
    presentation = _session_owner_source("sidebar-row-presentation.js")
    assert "function _sessionDisplayTitle" in display
    assert "function _sessionTitleTags" in display
    assert "function _sessionSearchDirectAndTitleMatches" in search
    assert "const titleMatches=(sessions||[]).filter(s=>_sessionDisplayTitle(s).toLowerCase().includes(q));" in search
    assert "const directAndTitleMatches=_sessionSearchDirectAndTitleMatches(sidebarStateBindings._allSessions,currentQ);" in search
    assert "const rawTitle=_sessionDisplayTitle(s);" in presentation
    assert "const tags=_sessionTitleTags(rawTitle);" in presentation
    assert "const segTitle=_sessionDisplayTitle(seg)||t('session_lineage_segment_untitled');" in presentation
    assert "const childTitle=_sessionDisplayTitle(child)||'Untitled child session';" in presentation
...
def test_streaming_state_recorded_from_own_state_not_bubbled_child():
    ...
    assert "const ownStreaming=_isSessionEffectivelyStreaming(s);" in presentation
    assert "const isStreaming=ownStreaming||!!s._child_session_streaming;" in presentation
    assert "_rememberRenderedStreamingState(s, ownStreaming);" in presentation
    # The old buggy pattern must not exist.
    assert "_rememberRenderedStreamingState(s, isStreaming);" not in presentation
...
def test_nested_fork_rows_included_in_visible_sidebar_ids():
    ...
    orchestrator = _session_owner_source("sidebar-list-orchestrator.js")
    assert "child.session_source==='fork'" in orchestrator
    # The _sessionVisibleSidebarIds builder must push fork children.
    assert "_sessionVisibleSidebarIds.push(child.session_id)" in orchestrator
...
def test_nested_fork_rows_render_select_checkbox():
    ...
    fork_render_start = presentation.find(render_marker)
    assert fork_render_start > 0
    fork_render_block = presentation[fork_render_start:fork_render_start + 2000]
