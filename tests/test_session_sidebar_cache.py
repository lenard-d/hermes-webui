...
from api.sessions import sidebar_cache as cache
...
def _isolated_session_list_cache_state():
    ...
    with cache._SESSIONS_CACHE_LOCK:
        cache._SESSIONS_CACHE_INFLIGHT.clear()
    yield
    cache._session_list_cache_clear()
    with cache._SESSIONS_CACHE_LOCK:
        cache._SESSIONS_CACHE_INFLIGHT.clear()
...
def test_session_list_cache_singleflight_rebuild_once(monkeypatch):
    cache._session_list_cache_clear()
    monkeypatch.setattr(cache, "_session_list_cache_source_stamp", lambda _key: ("stable",))
...
def test_session_list_cache_source_changed_owner_rebuilds_while_follower_reuses_stale(monkeypatch):
    ...
    cache._session_list_cache_set(key, _session_cache_payload("stale"))
    with cache._SESSIONS_CACHE_LOCK:
        ts, stamp, payload = cache._SESSIONS_CACHE[key]
        cache._SESSIONS_CACHE[key] = (
            ts - cache._SESSIONS_CACHE_TTL_SECONDS - 1.0,
            stamp,
            payload,
        )
...
def test_session_list_cache_owner_returns_stale_and_rebuilds_in_background(monkeypatch):
    ...
    try:
        ...
        cached, fresh = cache._session_list_cache_get(key, allow_stale=True)
...
def test_session_list_cache_stale_background_rebuild_failure_releases_owner(monkeypatch):
    ...
    for _ in range(20):
        with cache._SESSIONS_CACHE_LOCK:
            inflight_empty = key not in cache._SESSIONS_CACHE_INFLIGHT
        if inflight_empty:
            break
    ...
    for _ in range(20):
        cached, _fresh = cache._session_list_cache_get(key, allow_stale=True)
        if cached == _session_cache_payload("fresh"):
            break
        threading.Event().wait(0.05)
    assert cached == _session_cache_payload("fresh")
...
def test_session_list_cache_invalidated_on_session_list_publish():
    ...
    cache._session_list_cache_set(key_a, _session_cache_payload("a"))
    cache._session_list_cache_set(key_b, _session_cache_payload("b"))
    cache._session_list_cache_set(key_a_all, _session_cache_payload("a_all", all_profiles=True))

    session_events.publish_session_list_changed("session_pin", profile="profile-a")

    assert cache._session_list_cache_get(key_a)[0] is None
    assert cache._session_list_cache_get(key_a_all)[0] is None
    assert cache._session_list_cache_get(key_b)[0] is not None
...
def test_session_list_cache_rebuild_retries_after_invalidation():
    ...
    def builder():
        calls.append("build")
        if len(calls) == 1:
            cache._session_list_cache_clear("profile-a")
...
def test_session_list_cache_source_stamp_tracks_state_db_wal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cache, "_active_state_db_path", lambda: str(state_db))
    monkeypatch.setattr(cache, "_session_list_cache_gateway_session_metadata_path", lambda: gateway)
    monkeypatch.setattr(cache, "SESSION_DIR", session_dir)
    monkeypatch.setattr(cache, "SETTINGS_FILE", settings_file)
    ...
    before = cache._session_list_cache_source_stamp(key)
    state_db_wal.write_text("wal-2-more", encoding="utf-8")
    after = cache._session_list_cache_source_stamp(key)
...
def test_session_list_payload_to_response_overlays_live_stream_runtime(monkeypatch):
    ...
    monkeypatch.setattr(cache, "_active_stream_ids", lambda: {"live-stream"})
    with routes.LOCK:
...
def test_source_stamp_freezes_during_streaming_message_writes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cache, "_active_stream_ids", lambda: {"turn-1"})
...
def test_source_stamp_changes_when_stream_set_transitions(tmp_path, monkeypatch):
    ...
    streams = {"value": set()}
    monkeypatch.setattr(cache, "_active_stream_ids", lambda: set(streams["value"]))

    idle = cache._session_list_cache_source_stamp(key)
    streams["value"] = {"turn-1"}
    streaming = cache._session_list_cache_source_stamp(key)
    streams["value"] = {"turn-1", "turn-2"}
    two_streams = cache._session_list_cache_source_stamp(key)
    streams["value"] = set()
    idle_again = cache._session_list_cache_source_stamp(key)
...
def test_source_stamp_still_tracks_wal_when_idle(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cache, "_active_stream_ids", lambda: set())
...
def _age_cache_entry(key, seconds):
    ...
    with cache._SESSIONS_CACHE_LOCK:
        ts, stamp, payload = cache._SESSIONS_CACHE[key]
        cache._SESSIONS_CACHE[key] = (ts - seconds, stamp, payload)
...
def test_streaming_widens_cache_freshness_window(monkeypatch):
    ...
    monkeypatch.setattr(cache, "_session_list_cache_source_stamp", lambda k: ("stable",))

    cache._session_list_cache_set(key, _session_cache_payload("live"))
    # Age it past the idle TTL but keep it under the streaming TTL.
    _age_cache_entry(key, cache._SESSIONS_CACHE_TTL_SECONDS + 1.0)

    # Idle (no active stream) → stale → miss.
    monkeypatch.setattr(cache, "_active_stream_ids", lambda: set())
    payload_idle, fresh_idle = cache._session_list_cache_get(key)
    ...
    _age_cache_entry(key, cache._SESSIONS_CACHE_TTL_SECONDS + 1.0)
    monkeypatch.setattr(cache, "_active_stream_ids", lambda: {"turn-1"})
    payload_stream, fresh_stream = cache._session_list_cache_get(key)
...
def test_streaming_window_still_evicts_past_streaming_ttl(monkeypatch):
    ...
    cache._session_list_cache_set(key, _session_cache_payload("old"))
    _age_cache_entry(key, cache._SESSIONS_CACHE_STREAMING_TTL_SECONDS + 1.0)
    payload, fresh = cache._session_list_cache_get(key)
...
def test_streaming_window_does_not_extend_idle_ttl(monkeypatch):
    ...
    cache._session_list_cache_set(key, _session_cache_payload("idle"))
    _age_cache_entry(key, cache._SESSIONS_CACHE_TTL_SECONDS + 0.5)
    payload, fresh = cache._session_list_cache_get(key)
    assert payload is None and fresh is False
