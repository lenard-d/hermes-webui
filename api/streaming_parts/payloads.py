...
def session_payload_with_full_messages(
    api: ModuleType,
    session,
    *,
    tool_calls=None,
    ...
    api.attach_todo_state(raw, messages)
    if tool_calls is not None:
        raw["tool_calls"] = tool_calls
    return raw


def compact_for_echo_compare(value: str) -> str:
    """Normalize visible stream text for duplicate echo detection."""
    return re.sub(r"\s+", "", str(value or ""))


def strip_compact_echo_suffix(
    api: ModuleType,
    value: str,
    suffix: str,
    *,
    ...
    raw = str(value or "")
    candidate = api._compact_for_echo_compare(suffix)
    ...
    for idx in range(len(tail) + 1):
        if api._compact_for_echo_compare(tail[idx:]) == candidate:
            return raw[: offset + idx].rstrip(), True
    return raw, False


def redacted_session_payload_with_full_messages(
    api: ModuleType,
    session,
    *,
    tool_calls=None,
    ...
    try:
        return api.redact_session_data(
            api._session_payload_with_full_messages(
                session,
