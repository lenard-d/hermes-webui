...
def register_runtime_stream(
    ...
    *,
    ...
    _config_module.RUNTIME_STATE.register_stream(
        stream_id,
        session_id,
        channel,
...
def blocking_runtime_stream(
    ...
    *,
    ...
    return _config_module.RUNTIME_STATE.blocking_stream_for_session(
        session_id,
...
def runtime_stream_alive(stream_id: str) -> bool:
    return _config_module.RUNTIME_STATE.has_stream(stream_id)


def runtime_worker_alive(stream_id: str) -> bool:
    return _config_module.RUNTIME_STATE.has_worker(stream_id)


def runtime_transport(stream_id: str):
    return _config_module.RUNTIME_STATE.transport(stream_id)


def runtime_transport_items():
    return _config_module.RUNTIME_STATE.transport_items()


def runtime_transport_count(*, timeout: float | None = None) -> int | None:
    return _config_module.RUNTIME_STATE.transport_count(timeout=timeout)


def runtime_worker_items():
    return _config_module.RUNTIME_STATE.worker_items()


def runtime_last_run_finished_at() -> float | None:
    return _config_module.RUNTIME_STATE.last_run_finished_at


def runtime_active_run_ids():
    return _config_module.RUNTIME_STATE.active_run_ids()


def runtime_run_session_id(stream_id: str) -> str | None:
    return _config_module.RUNTIME_STATE.run_session_id(stream_id)


def runtime_last_event_id(stream_id: str) -> str | None:
    return _config_module.RUNTIME_STATE.last_event_id(stream_id)


def note_runtime_last_event_id(stream_id: str, event_id: str) -> None:
    _config_module.RUNTIME_STATE.note_last_event_id(stream_id, event_id)
...
def initialize_runtime_execution(stream_id: str, cancel_event) -> bool:
    ...
    return _config_module.RUNTIME_STATE.initialize_execution(stream_id, cancel_event)


def append_runtime_partial_text(stream_id: str, text) -> bool:
    return _config_module.RUNTIME_STATE.append_partial_text(stream_id, text)


def replace_runtime_partial_text(stream_id: str, text) -> bool:
    return _config_module.RUNTIME_STATE.replace_partial_text(stream_id, text)


def append_runtime_reasoning_text(stream_id: str, text) -> bool:
    return _config_module.RUNTIME_STATE.append_reasoning_text(stream_id, text)


def replace_runtime_reasoning_text(stream_id: str, text) -> bool:
    return _config_module.RUNTIME_STATE.replace_reasoning_text(stream_id, text)
...
def start_runtime_tool_call(
    ...
    *,
    ...
    return _config_module.RUNTIME_STATE.start_tool_call(
        stream_id,
        name=name,
        args=args,
...
def finish_runtime_tool_call(
    ...
    **metadata,
    ...
    return _config_module.RUNTIME_STATE.finish_tool_call(
        stream_id,
        name=name,
        tool_call_id=tool_call_id,
        **metadata,
