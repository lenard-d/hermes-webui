...
//
// The cache owns its accounting and eviction policy. Callers only provide a
// session id and a render entry; they do not mutate the backing Map directly.
(function(){
  const ROOT=(typeof window!=='undefined')?window:globalThis;

  function _positiveLimit(value, fallback){
    const parsed=Number(value);
    return Number.isFinite(parsed)&&parsed>0?Math.floor(parsed):fallback;
  }

  function create(options={}){
    const maxEntries=_positiveLimit(options.maxEntries,8);
    const maxEntryBytes=_positiveLimit(options.maxEntryBytes,2*1024*1024);
    const maxTotalBytes=_positiveLimit(options.maxTotalBytes,8*1024*1024);
  ...
  ROOT.HermesSessionRenderCache=Object.freeze({create});
})();
